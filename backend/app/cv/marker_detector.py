...
def detect_markers(image: np.ndarray) -> DetectionResult:
    ...
    detector = cv2.aruco.ArucoDetector(april_tag_dictionary(), parameters)
    ...
    for marker_corners, marker_id in zip(corners, ids.flatten(), strict=True):
        refined = marker_corners.reshape(4, 2).astype(np.float32)
        cv2.cornerSubPix(gray, refined, (3, 3), (-1, -1), criteria)
