...
export default function PhoneFrame({ activeCase, step, measurement, review, processMessage, processBusy, captures, decision, observerName, remark, record, liveAIState, sessionMode, onPrimary, onDecision, onObserverName, onRemark, onRunLiveAI, onOpenRecord }: PhoneFrameProps) {
  ...
  return (
    <section id="judge-phone" className="showcase-phone-column" aria-label="巡查员手机模拟器" data-testid="showcase-phone">
      <div className="phone-mode-label"><span><i aria-hidden="true" />评委操作区</span><strong>手机 UI 实时同步</strong><small>直接点击手机内绿色按钮</small></div>
      <div className="phone-frame"><div className="phone-hardware"><span /></div><div className="phone-screen">
        <div className="phone-status"><span><i className="phone-sync-dot" aria-hidden="true" />实时同步</span><b>GeoReCheck</b><span>▮▮▮</span></div>
        ...
        {isProcessing ? <div className="phone-content phone-processing"><div className="processing-orbit"><span /><i /></div><span className="phone-kicker">{step.id === "geometry" ? "FastAPI / OpenCV" : "AI 实测回放"}</span><h2>{processMessage || "正在建立可审计结果"}</h2><ul><li className={measurement ? "done" : "active"}><b>{measurement ? "✓" : "…"}</b>真实几何接口</li><li className={measurement ? "done" : "pending"}><b>{measurement ? "✓" : "02"}</b>质量门控</li><li className={review ? "done" : step.id === "ai_review" ? "active" : "pending"}><b>{review ? "✓" : "03"}</b>StepFun 成功响应回放</li><li className={review ? "done" : "pending"}><b>{review ? "✓" : "04"}</b>{sessionMode ? "浏览器会话" : "SQLite AI 条目"}</li></ul><small>{processBusy ? "正在等待实时处理…" : sessionMode ? "几何实时运行；AI 回放与人工决定保存在当前会话。" : "几何与记录是真实运行；AI 内容来自已保存的真实调用。"}</small></div> : null}

        {step.id === "result" ? <div className="phone-content phone-result"><span className={`quality-chip ${qualityPassed ? "passed" : "failed"}`}>{qualityPassed ? "图片质量通过" : "图片质量不合格"}</span><div className="phone-opening"><small>裂缝较上次变化</small><strong>{opening == null ? "未输出" : `${opening >= 0 ? "+" : ""}${opening.toFixed(1)} mm`}</strong><span>实时 OpenCV 几何测量</span></div><BeforeAfterSlider activeCase={activeCase} /><div className="phone-ai-result"><span>AI 实测回放</span><strong>{qualityPassed ? aiLabel : "图片覆盖不足"}</strong><p>{qualityPassed ? aiEvidence : activeCase.geometry.quality_reasons.join("；")}</p><small>{activeCase.ai_replay.provider} · {activeCase.ai_replay.model} · 原始 {(activeCase.ai_replay.original_latency_ms / 1000).toFixed(1)} s · {activeCase.ai_replay.validated_date}</small></div><div className="live-ai-callout" data-testid="live-ai-callout"><div><span><i aria-hidden="true" />{sessionMode ? "STABLE" : "LIVE API"}</span><strong>{sessionMode ? "稳定路演模式" : "可选：现场运行实时 StepFun"}</strong></div><p>{sessionMode ? "采用已审计 StepFun 回放；现场几何仍实时运行。" : "约 30–60 秒；失败也不会覆盖几何结果。"}</p>{sessionMode ? <small>实时 AI 请使用本地一键启动版。</small> : <button className="live-ai-button" onClick={onRunLiveAI} disabled={!measurement || !qualityPassed || liveAIState === "running" || liveAIState === "completed"}>{liveAIState === "running" ? "实时 AI 运行中…" : liveAIState === "completed" ? "实时 AI 已完成" : liveAIState === "failed" ? "实时 AI 失败，可重试" : "运行实时 AI（约 30–60 秒）"}</button>}</div><button onClick={onPrimary}>进入人工确认</button></div> : null}

        {step.id === "human_confirm" ? <div className="phone-content phone-confirm"><span className="human-waiting">自动演示已暂停 · 轮到你操作</span><h2>{qualityPassed ? aiLabel : "本次不能形成毫米结果"}</h2><div className="confirm-evidence"><img src={activeCase.assets.current_close} alt="待人工确认的本次近景" /><p>{qualityPassed ? aiEvidence : activeCase.geometry.quality_reasons.join("；")}</p></div><p className="confirm-guide"><b>1</b>选择“确认”或“不采纳” <b>2</b>填写信息 <b>3</b>生成记录</p>{qualityPassed ? <div className="phone-decision"><button className={decision === "accepted" ? "selected" : ""} onClick={() => onDecision("accepted")}>确认</button><button className={decision === "rejected" ? "selected" : ""} onClick={() => onDecision("rejected")}>不采纳</button></div> : null}<button type="button" className="demo-fill-button" onClick={() => { onObserverName("路演评委"); onRemark("现场已核对照片与可见变化，复测贴完整。"); }}>一键填入演示信息</button><label>监测员姓名<input aria-label="监测员姓名" value={observerName} onChange={(event) => onObserverName(event.target.value)} placeholder="请输入姓名" /></label><label>现场备注<textarea aria-label="现场备注" value={remark} onChange={(event) => onRemark(event.target.value)} placeholder="输入现场核对情况" /></label><button className="primary-action" onClick={onPrimary} disabled={!observerName.trim() || !remark.trim() || (qualityPassed && decision === "pending")}>{qualityPassed ? "确认并生成记录" : "保存失败证据"}</button></div> : null}

        {step.id === "record" ? <div className="phone-content phone-record"><div className={`record-check ${qualityPassed ? "" : "rejected"}`}>{qualityPassed ? "✓" : "!"}</div><span className="phone-kicker">{qualityPassed ? "巡查记录已生成" : sessionMode ? "失败证据已保存" : "失败证据已落库"}</span><h2>{record?.id ?? measurement?.id}</h2><dl><div><dt>裂缝变化</dt><dd>{opening == null ? "未输出" : `${opening >= 0 ? "+" : ""}${opening.toFixed(1)} mm`}</dd></div><div><dt>监测员</dt><dd>{record?.observer_name ?? observerName}</dd></div><div><dt>人工决定</dt><dd>{decision === "accepted" ? `${aiLabel} · 已确认` : "AI 提示未采纳"}</dd></div><div><dt>现场备注</dt><dd>{record?.remark || remark || "无"}</dd></div></dl><p>{sessionMode ? "几何、AI 人工处置与输入内容已进入当前浏览器的路演会话记录。" : "几何、AI 人工处置与输入内容已进入同一条 SQLite 记录。"}</p><button onClick={onOpenRecord}>查看正式记录</button><div className="final-comparison"><span>传统：量 → 翻 → 比 → 拍 → 填</span><strong>GeoReCheck：拍 → 确认</strong><b>几何量毫米，AI补人眼，人来做确认。</b></div></div> : null}
      </div></div>
