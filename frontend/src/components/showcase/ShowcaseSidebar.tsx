...
export default function ShowcaseSidebar({ step, activeCase, isPlaying }: { step: FieldStep; activeCase: ShowcaseCase; isPlaying: boolean }) {
  ...
  return (
    <aside className="showcase-sidebar" aria-label="本次巡查步骤" data-testid="showcase-sidebar">
      ...
      <section className="inspection-facts"><h3>实时数据链</h3><div><b>LIVE</b><span>FastAPI / OpenCV 本次运行</span></div><div><b>AI</b><span>默认实测回放 · 结果页可实时运行</span></div><div><b>WRITE</b><span>本次巡查记录真实写入</span></div><div><b>人工</b><span>确认前绝不自动采纳</span></div></section>
